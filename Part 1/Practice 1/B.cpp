  ...
  int see[4*N],a[N];
    ...
    void build(int vx,int l,int r)
    {
      ...
      else
      {
        ...
        see[vx]=min(see[2*vx],see[2*vx+1]);
      }
    }
    
    int qry(int vx,int l,int r,int ql,int qr)
    { 
      if(ql>qr) return mod;
      if(l==ql&&r==qr) return see[vx];
      int mid=(l+r)>>1;
      return min(qry(2*vx,l,mid,ql,min(qr,mid)),qry(2*vx+1,mid+1,r,max(mid+1,ql),qr));
    }
